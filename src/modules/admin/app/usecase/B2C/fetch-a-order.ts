...
class FetchAOrder {
    ...
    private fetchProduct: (productId: string) => Promise<{
    ...
    async execute(data: Input): Promise<Output> {
        ...
        try {
            ...
            const productRes = await this.fetchProduct(order.productId)
            const product = productRes.response?.data
            if (!product) return {
                response: { message: productRes.response.message },
                status: productRes.status
            }


            //check ownership of the product
            if (String(admin._id) != String(product.ownership.ownerId)) return {
    ...
    private OrderMapper(user: User, order: Order, product: Product): MappedOrder {
...
interface Output {
    response: { message: string, data?: MappedOrder },
    status: StatusCode
}
