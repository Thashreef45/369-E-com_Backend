...
class CreateCategory {
    ...
    private createCategory: (data: { name: string, description: string }) => Promise<Output>
    ...
    async execute(data: Input): Promise<Output> {
        ...
        try {
            // create category
            const created = await this.createCategory({ name: data.name, description: data.description })
            return {
...
interface Dependencies {
    createCategory(data: { name: string, description: string }): Promise<Output>
}
